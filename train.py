...
def main():
    ...
    lr_schedules = [opt.lr_update, opt.lr_update+5]
...
def train(opt, train_loader, model, epoch, val_loader, best_rsum):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # rg_features_batch, rg_lengths, relative_boxes_batch, gd_features_batch, al_graphs_batch, targets, lengths = train_data
        detections, rg_lengths, boxes, grids, masks, captions, lengths, _, img_ids = train_data

        model.train_emb(detections, rg_lengths, boxes, grids, masks, captions, lengths, img_ids)
...
def adjust_learning_rate(opt, optimizer, epoch, lr_schedules):
    ...
    if epoch <=4 :
        ...
        new_lr = base_lr * (epoch+1) / 5
        logger.info('Current epoch num is {}, all lrs are warmed up with {}'.format(epoch, new_lr))
        for i, param_group in enumerate(optimizer.param_groups):
            if i == 1:
                ...
                new_lr1 = new_lr
                param_group['lr'] = new_lr1
            logger.info('new lr {}'.format(new_lr1))

    elif epoch in lr_schedules:
