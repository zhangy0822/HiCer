...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            # images, image_lengths, captions, lengths, ids, clip_images, image_raw = data_i
            # images, image_lengths, captions, lengths, ids, clip_images = data_i
            detections, rg_lengths, boxes, grids, masks, captions, lengths, ids, img_ids = data_i
        else:
        ...
        if not backbone:
            img_emb, cap_emb = model.forward_emb(detections, rg_lengths, boxes, grids, masks, captions, lengths, img_ids)
        else:
...
def eval_ensemble_cxc(results_paths):
    
    import os
    import json
    cxc_annot_base = os.path.join('/root/pan/netDiskS/zy/dataset/coco', 'cxc_annots')
    ...
    all_sims = []
    all_npts = []
    all_sims_i, all_sims_t = [], []
    for sim_path in results_paths:
        fr = open(sim_path,'rb')
        results = pickle.load(fr)
        npts = results['npts']
        sims = results['sims']
        sims_i = results['sims_i']
        sims_t = results['sims_t']
        all_npts.append(npts)
        all_sims.append(sims)
        all_sims_i.append(sims_i)
        all_sims_t.append(sims_t)
    all_npts = np.array(all_npts)
    all_sims = np.array(all_sims)
    all_sims_i = np.array(all_sims_i)
    all_sims_t = np.array(all_sims_t)
    assert np.all(all_npts == all_npts[0])
    npts = int(all_npts[0])
    sims = all_sims.mean(axis=0)
    sims_i = all_sims_i.mean(axis=0)
    sims_t = all_sims_t.mean(axis=0)
    ...
    i2i_recalls = cxc_intra(images, img_ids, cxc_i2i, sims_i = sims_i)
    t2t_recalls = cxc_intra(captions, cap_ids, cxc_t2t, text=True, sims_i = sims_t)
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    logger.info(checkpoint['best_rsum'])
    ...
    # construct model
    model_test = VSEModel(opt)

    # model_test.make_data_parallel()
    # load model state
    model_test.load_state_dict(checkpoint['model'])
    model_test.val_start()
    ...
    with torch.no_grad():
        if opt.precomp_enc_type == 'basic':
            img_embs, cap_embs = encode_data(model_test, data_loader)
        else:
            img_embs, cap_embs = encode_data(model_test, data_loader, backbone=True)
    logger.info('Images: %d, Captions: %d' %
                (img_embs.shape[0] / 5, cap_embs.shape[0]))

    if cxc:
        eval_cxc(img_embs, cap_embs, data_path, save_path)
    else:
        if not fold5:
            ...
            logger.info("Average t2i Recall: %.1f" % ari)
            logger.info("Text to image: %.1f %.1f %.1f %.1f %.1f" % ri)
            torch.save({'rt': rt, 'rti': rti}, 'ranks_{}_top5.pth.tar'.format(opt.data_name))
        else:
...
def i2t(npts, sims, return_ranks=False, mode='coco'):
    ...
    if return_ranks:
        return (r1, r5, r10, medr, meanr), (ranks, top5)
    else:
...
def eval_cxc(images, captions, data_path, save_path):
    import os
    import json, pickle
    ...
    sims_i = compute_sim(images, images)
    sims_t = compute_sim(captions, captions)
    
    npts = images.shape[0]
    if save_path is not None:
        # np.save(save_path, {'npts': npts, 'sims': sims, 'sims_i': sims_i, 'sims_t': sims_t})
        with open(save_path, "wb") as writer:
            pickle.dump({'npts': npts, 'sims': sims, 'sims_i': sims_i, 'sims_t': sims_t},writer, protocol=4)
        logger.info('Save the similarity into {}'.format(save_path))
...
def cxc_intra(embs, data_ids, annot, text=False, sims_i = None):
